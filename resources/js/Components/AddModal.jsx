...
export default function AddModal({
    ...
    const handleCategoriaChange = ({ target: { value } }) => {
        setFormData(prev => ({ ...prev, id_categoria: value === "nueva" ? null : value }));
        setMostrarNuevaCategoria(value === "nueva");
    };

    const handleProveedorChange = ({ target: { value } }) => {
        setFormData(prev => ({ ...prev, id_proveedor: value === "nuevo" ? null : value }));
    ...
    const handleSubmit = (e) => {
        ...
        router.post(route("producto.patch"), formData, {
            onSuccess: () => {
                showModificableAlert("Producto añadido", "Se agregó el producto al inventario.", "success");
        ...
        <Dialog open={isOpen} onClose={onClose} className="fixed inset-0 z-50 flex items-center justify-center">
            <div className="bg-white rounded-lg shadow-lg p-6 w-full max-w-md">
                ...
                <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                    ...
                    <button type="submit" className="bg-slate-500 text-white px-4 py-2 rounded-md font-semibold hover:bg-slate-600">
                        Guardar
                    </button>
                </form>
            </div>
